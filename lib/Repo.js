...
class Repo {
    ...
    async hashWorkTree (files = null) {
        ...
        await git.add(holoIndexEnv, ...files ? files : [{ all: true }]);
        return git.writeTree(holoIndexEnv);
    }
    ...
    async watch ({ callback }) {
        ...
        if (this.workTree) {
            ...
            const hashDirtyPaths = debounce(async () => {
                const pathsArray = [...dirtyPaths];
                dirtyPaths.clear();
                callback(await this.hashWorkTree(pathsArray));
            }, 50);

            client.on('subscription', async (response) => {
                for (const file of response.files) {
                    dirtyPaths.add(file.name);
                }
                hashDirtyPaths();
            });
            ...
            watcher.on('change', async ref => {
                ...
                if (refHash != lastRefHash) {
                    lastRefHash = refHash;
                    const treeHash = await git.revParse({ verify: true }, `${refHash}^{tree}`);
