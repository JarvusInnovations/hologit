...
class Repo {
    ...
    static async getFromEnvironment ({ ref = 'HEAD', working = false } = {}) {
        ...
        let caches = instanceCache.get(gitDir);
        const cacheKey = working ? `working:${workTree}` : `ref:${ref}`;
        const cachedRepo = caches && caches.get(cacheKey);
        ...
        if (!caches) {
            caches = new Map();
            instanceCache.set(gitDir, caches);
        }

        caches.set(cacheKey, repo);
    ...
    async getConfig () {
        ...
        if (!config) {
            throw new Error('.holo/config.toml does not exist, initialize with \`git holo init\`');
        }
