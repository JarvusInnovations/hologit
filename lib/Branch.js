        ...
        for (const key in mappings) {
            ...
            if (after) {
                for (const layer of after) {
                    if (!mappingsByLayer[layer]) {
                        throw new Error(`layer ${layer} not found, configured as 'after' requirement for mapping ${key}`);
                    }
            ...
            if (before) {
                for (const layer of before) {
                    if (!mappingsByLayer[layer]) {
                        throw new Error(`layer ${layer} not found, configured as 'before' requirement for mapping ${key}`);
                    }
