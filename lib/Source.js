...
class Source extends Configurable {
    ...
    async getConfig () {
        ...
        if (!config.local) {
            ...
            const { hash: specHash } = await Source.buildSpec(this.repo, config);
            config.spec = specHash;
            config.specRef = `refs/holo/sources/${specHash}`;
        }
    ...
    async getCachedHead ({ refresh=false } = {}) {
        const cachedHead = !refresh && headCache.get(this);

        if (cachedHead) {
            return cachedHead;
        }

        const head = await this.getHead(...arguments);
