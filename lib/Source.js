...
class Source extends Configurable {
    ...
    async checkoutSubmodule () {
        ...
        await fs.writeFile(submoduleAlternatesPath, path.relative(`${submoduleGitDir}/objects`, `${gitDir}/objects`));
        ...
        let shallowList;
        if (await fs.exists(shallowPath)) {
            const shallowContent = await fs.readFile(shallowPath);
            shallowList = new Set(shallowContent.toString().trim().split('\n'));
        } else {
            shallowList = new Set();
        }
        shallowList.add(head);
