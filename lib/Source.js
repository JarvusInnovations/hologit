...
class Source extends Configurable {
    ...
    async getHead ({ required=false } = {}) {
        ...
        if (!head) {
            head = await this.repo.resolveRef(`${this.repo.ref}:.holo/sources/${this.name}`);
        }
    ...
    async getSubmodule ({ required=false } = {}) {
        ...
        const workTree = this.repo.workTree && path.join(this.repo.workTree, submodulePath);
