...
class TreeObject {
    ...
    async deleteChild (childPath) {
        const tree = await this.getSubtree(path.dirname(childPath), false);
        if (!tree) {
            return
        }

        const childName = path.basename(childPath);
        if (tree._children[childName] || !tree._baseChildren) {
            tree._children[childName] = null;
            tree.markDirty();
        }
    }
    ...
    async clone () {
        return new TreeObject(this.repo, { hash: await this.getHash(), parent: this.parent });
    }
}
