...
class Lens extends Configurable {
    ...
    async buildSpec (inputTree) {
        ...
        const [,,, localPkgBuild] = localPkgIdent.split('/');
        ...
        try {
            ...
            // TODO: test if newer
            if (localPkgBuild > pkgIdentArray[3]) {
            ...
            if (err.response.status == 404 && localPkgIdent) {
                logger.warn(`lens package ${packageQuery} not found in habitat depot, falling back to local install...`);
