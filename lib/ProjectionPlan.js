...
class ProjectionPlan {
    ...
    async project (options = {}) {
        // Delegate pure composition to the Rust engine when eligible
        // (specs/behaviors/engine-selection.md); lensing plans stay on the
        // JS path, which owns lens execution
        if (!options.lens) {
            const RustEngine = require('./RustEngine.js');
            const rustHash = await RustEngine.projectPlan(this.repo, this._sources, this._mappings, options);
            if (rustHash) {
                return rustHash;
            }
        }
