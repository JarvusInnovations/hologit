...
/**
 * Get js-git repository instance
 */
Git.prototype.getRepo = function(...modules) {
    var repo;

    if (this.repo) {
        repo = Promise.resolve(this.repo);
    } else {
        repo = this.exec('rev-parse', { 'git-dir': true }).then(gitDir => {
            this.repo = {
                _loadedModules: [
                    'git-node-fs/mixins/fs-db',
                    'js-git/mixins/fs-db'
                ]
            };
            require('git-node-fs/mixins/fs-db')(this.repo, gitDir);
            return this.repo;
        });
    }

    return repo.then(repo => {
        var loadedModules = repo._loadedModules,
            length = modules.length, i = 0, module;
...
Git.prototype.exec = function(command, options, args) {
    ...
    while (execArgs.length) {
        ...
        switch (typeof args) {
            ...
            case 'object':
                ...
                if ('$preserveEnv' in args) {
                    execOptions.preserveEnv = args.$preserveEnv;
                    delete args.$preserveEnv;
                }
    ...
    if (execOptions.preserveEnv !== false) {
        Object.setPrototypeOf(gitEnv, process.env);
    }
 ...
 */
Git.getGitDirFromEnvironment = function() {
    return Git.prototype.exec('rev-parse', { 'git-dir': true }).then(function(gitDir) {
 ...
 */
Git.getWorkTreeFromEnvironment = function() {
    return Git.prototype.exec('rev-parse', { 'show-toplevel': true }).then(function(workTree) {
        return workTree ? fs.realpath(workTree) : Promise.resolve(null);
    });
};