...
Git.prototype.exec = function(execOptions, command, options, args, callback) {
    ...
    if (execOptions.spawn) {
        ...
        return child_process.exec('git ' + gitArgs.join(' '), execOptions, callback ? function (error, stdout, stderr) {
            gitArgs;
            callback(error, stdout.trim());
        } : null);
    } else {
        return child_process.execFile('git', gitArgs, execOptions, callback ? function (error, stdout, stderr) {
            callback(error, stdout.trim());
        } : null);
    }
};
