...
3. **Source resolution** — each mapping's holosource resolves in this order: self-source (the repo projecting itself) → gitlink (submodule pointer) → spec-ref (`refs/holo/source/...`) → local ref from the source's configured `ref` (url-less sources only). Refs that point at annotated tags are peeled to commits. Resolution is pure; the fetch capability that populates spec-refs, and the full resolution semantics, are specced in [`source-resolution.md`](source-resolution.md). A `source=>holobranch` or `=>holobranch` reference triggers **recursive sub-projection**: the referenced holobranch is projected first (against the source's tree or the current tree respectively) and its output tree becomes the mapping's input.
